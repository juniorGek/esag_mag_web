...
// Données de démonstration pour les polls
const pollsData = [
  {
    id: 1,
...
export default function PollsTable() {
  ...
  const [selectedPolls, setSelectedPolls] = useState(null); // Pour stocker le sondage sélectionnée
  ...
  // Filtrage des sondages en fonction de la recherche
  ...
  const pageCount = Math.ceil(filteredPolls.length / rowsPerPage);
  const offset = currentPage * rowsPerPage;
  const currentPolls = filteredPolls.slice(offset, offset + rowsPerPage);

  // Ouvrir la modale d'édition
  const openEditModal = (item) => {
    setSelectedPolls(item);
  ...
  // Gérer la modification d'un sondage
  const handleEdit = (e) => {
    e.preventDefault();
    const updatedPolls = polls.map((item) =>
      item.id === selectedPolls.id ? { ...item, ...selectedPolls } : item
    );
    setPolls(updatedPolls);
    closeModals();
  };

  // Gérer la suppression d'un sondage
  const handleDelete = () => {
    const updatedPolls = polls.filter((item) => item.id !== selectedPolls.id);
    setPolls(updatedPolls);
    closeModals();
  };
      ...
      <form onSubmit={handleEdit}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={selectedPolls.titre}
              onChange={(e) =>
                setSelectedPolls({ ...selectedPolls, titre: e.target.value })
              }
          ...
          <div>
            ...
            <input
              type="text"
              value={selectedPolls.auteur}
              onChange={(e) =>
                setSelectedPolls({ ...selectedPolls, auteur: e.target.value })
              }
              className="w-full p-2 border border-gray-300 rounded-lg dark:bg-slate-700 dark:text-slate-50"
              required
            />
          ...
          <div>
            ...
            <textarea
              value={selectedPolls.description}
              onChange={(e) =>
                setSelectedPolls({ ...selectedPolls, description: e.target.value })
              }
              className="w-full p-2 border border-gray-300 rounded-lg dark:bg-slate-700 dark:text-slate-50"
              rows="4"
              required
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-4">
              ...
              {selectedPolls.imageCover && (
                <div className="w-16 h-16 rounded-lg overflow-hidden">
                  <img
                    src={selectedPolls.imageCover}
          ...
          <div>
            ...
            <select
              value={selectedPolls.enabled ? "Activé" : "Désactivé"}
              onChange={(e) =>
                setSelectedPolls({
                  ...selectedPolls,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <p className="text-gray-700 dark:text-slate-300 mb-6">
              Êtes-vous sûr de vouloir supprimer ce sondage ?
            </p>
